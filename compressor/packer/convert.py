...
def linear_w4a8(
    ...
    return {
        "qweight": qweight,
        "scales_0": scale_0.half(),
        "scales_1": scale_1.half(),
        "zeros": zero.half()
    }
...
def convert_linear(
    ...
    if format_type == "w4a16":
        ...
        return linear_w4a16(weight, scale, zero, args)
        ...
        return linear_w4a8(weight, scale_0, scale_1, zero, args)
    
    elif format_type == "w8a8":
        scale = qparams[f"{qparams_key}.scale"]
        zero = qparams[f"{qparams_key}.zero"]

        return linear_w8a8(weight, scale, zero, args)
